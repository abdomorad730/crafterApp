...
export class ProductService {
    ...
    async createProduct(body:ProductDto,user:UserDocument,files:{imageCover?: Express.Multer.File[], images?: Express.Multer.File[] }){
        ...
        const BrandExist=await this.BrandRepository.findOne({name:brand})
        if(!BrandExist ){
            throw new NotFoundException('brand not found')

        }
        const SubCategoryExist=await this.SubCategoryRepository.findOne({name:subCategory})
        if(!SubCategoryExist ){
    ...
    async getSpecificProducts(id:Types.ObjectId){

        const product=await this.ProductRepository.findOne({_id:id},[{path:'category'},{path:'brand'},{path:'subCategory'}])
        if(!product){
            throw new NotFoundException('product not found')
        }
        return {product}
