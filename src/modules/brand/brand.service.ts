...
export class BrandService {
    ...
    async createBrand(body: brandDto, user: UserDocument, file: Express.Multer.File) {
        ...
        let dummyData = { name, userId: user._id, category:categoryExist._id, customId }
    ...
    async getSpecificBrand(id: Types.ObjectId) {
        const Brand = await this.BrandRepository.findOne({ _id: id })
        if(!Brand){
            throw new NotFoundException('Brand not found')
        }
        const subCategories = await this.ProductRepository.find({ filter: { Brand: id }, populate: [{ path: 'Brand' }] })
        return { products: subCategories, Brand }
    }
}
