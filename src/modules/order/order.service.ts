...
import { decrypt } from 'src/common/service/crypto';
...
export class OrderService {
    ...
    async createOrder(body: orderDto, user: UserDocument) {
        ...
        const order = await this.OrderRepository.create({
            userId: user._id,
            cartId: cart._id,
            phone:decrypt(user.phone,process.env.encrypt_key),
            paymentMethod,
    ...
    async createPaymentWithStripe(body:paymentDto,user:UserDocument){
        ...
        if(code){
           ...
           const coupon=await this.paymentService.createCoupon(couponExist.amount)
            const data=await this.paymentService.createCheckoutSession({
                ...
                success_url:'http://localhost:5173/allorders',
        ...
        const data=await this.paymentService.createCheckoutSession({
            customer_email:user.email,
            metadata:{orderId:orderId.toString()},
            success_url:'http://localhost:5173/allorder',
            cancel_url:'http://localhost:5173',
            line_items: order.cartId['products'].map(product=>({
                price_data:{
                    currency:"egp",
    ...
    async getAllorders(){
        const orders=await this.OrderRepository.find({populate:[{path:'userId'}]})
        return {orders}
    }
    async getspecificOrders(id:Types.ObjectId){
        const order=await this.OrderRepository.findOne({_id:id},[{path:'userId'}])
        if(!order){
            throw new NotFoundException('order not found')
        }
        return {order}
