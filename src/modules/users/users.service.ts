...
import { userOtp, userRoles } from "src/common/types/types";
...
export class userService {
    ...
    async SignUp(body: userDto,file:Express.Multer.File ): Promise<any> {
        try {
            ...
            let image:object={};
            if(file){
                const{secure_url,public_id} =await this.UploadedFileService.uploadFile(file,{folder:`${process.env.folder}/users/imageCover`})
                 image={secure_url,public_id}
            }
            const user = await this.userRepository.create({ firstName, lastName, email, password, gender, address, phone, DOB,role:userRoles.crafter,image })
            const code = Math.floor(Math.random() * (999 - 8) + 254).toString()
            sendEmail({ to: email, subject: 'confirm email', html: `<h1>${code}</h1>` })
            await this.OTPRepository.createOtp({ otp:hash(code,12), userId: user['_id'], expireAt: new Date(Date.now() + 1000 * 60 * 60), type: userOtp.email })
    ...
    async SignUpAsUser(body: userDto,file:Express.Multer.File ): Promise<any> {
        try {
            ...
            const user = await this.userRepository.create({ firstName, lastName, email, password, gender, address, phone, DOB,role:userRoles.user,image })
            const code = Math.floor(Math.random() * (999 - 8) + 254).toString()
            sendEmail({ to: email, subject: 'confirm email', html: `<h1>${code}</h1>` })
            await this.OTPRepository.createOtp({ otp:hash(code,12), userId: user['_id'], expireAt: new Date(Date.now()+1000 * 60 * 60 ), type: userOtp.email })
            if(file){
    ...
    async SignIn(body: any): Promise<any> {
        try {
            ...
            return { msg: 'done', access_token, refresh_token , user:{ email, id: userExist['_id'],role:userExist.role ,name:userExist.firstName+" "+userExist.lastName} }
        } catch (error) {
            throw new InternalServerErrorException(error)
        }
    }
